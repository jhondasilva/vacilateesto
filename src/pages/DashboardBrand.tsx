...
import {
  ...
  Youtube, Instagram, Play, ExternalLink, MessageCircle, Music2, Facebook, RefreshCw, Info,
} from "lucide-react";
              ...
              {postsForView.map((p) => {
                ...
                return (
                  ...
                  >
                    ...
                    {p.url?.includes('/stories/') && (
                      <div className="absolute top-2 right-2 bg-background/85 backdrop-blur rounded-full px-2 py-0.5 text-[9px] font-bold uppercase tracking-wider">
                        Story
                      </div>
                    )}
                    <div className="absolute inset-x-0 bottom-0 p-2 bg-gradient-to-t from-black/85 via-black/55 to-transparent text-white">
                      ...
                      {p.url?.includes('/stories/') ? (
                        <div className="flex items-center justify-between text-[10px] font-bold">
                          <span className="flex items-center gap-1"><Eye className="w-3 h-3" />{fmtNum((p.metrics.impressions || p.metrics.views) ?? 0)}</span>
                          <span className="flex items-center gap-1 opacity-90">Alcance {fmtNum(p.metrics.reach ?? 0)}</span>
                        </div>
                      ) : (
                        <div className="flex items-center justify-between text-[10px] font-bold">
