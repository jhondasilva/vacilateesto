...
Uso:
    ./run-status.sh               # snapshot único
    ./run-status.sh --watch       # refresca cada 30s
    python3 status.py --watch --interval 10
"""
import os
import sys
import time
...
# ── main ────────────────────────────────────────────────────────
def main():
    parser = argparse.ArgumentParser(description="Status de diarización de Vacílate El Podcast")
    parser.add_argument("-w", "--watch", action="store_true", help="Modo watch: refresca automáticamente")
    parser.add_argument("-i", "--interval", type=int, default=30, help="Segundos entre refrescos (default: 30)")
    ...
    if args.watch:
        try:
            while True:
                ...
                print(f"{C.DIM}⏱  Última actualización: {now}  |  Intervalo: {args.interval}s  |  Ctrl+C para salir{C.RST}\n")
                print_status()
                # countdown visual
                for remaining in range(args.interval, 0, -1):
                    sys.stdout.write(f"\r{C.DIM}Próximo refresco en {remaining}s...{C.RST}  ")
                    sys.stdout.flush()
                    time.sleep(1)
                sys.stdout.write("\r" + " " * 40 + "\r")
                sys.stdout.flush()
        except KeyboardInterrupt:
            print(f"\n\n{C.OK}👋  Modo watch detenido.{C.RST}")
            sys.exit(0)
    else:
        print_status()
